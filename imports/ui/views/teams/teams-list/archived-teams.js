...
Template.archivedTeams.onRendered(function () {
    ...
    var table = $('#archived-teams-table').DataTable();
...
Template.archivedTeams.events({
    'click #select-all': function (event, tmpl) {
        ...
        var allChecked = $('tbody input[type="checkbox"]', '#archived-teams-table').prop('checked', this.checked);
    ...
    'click #restore-teams': function (event, tmpl) {
        ...
        Meteor.call('restoreTeams', checkedTeams, function (error, result) {
            if(!error){
                VZ.notify('Restored');
                tmpl.checkedTeams.clear();
