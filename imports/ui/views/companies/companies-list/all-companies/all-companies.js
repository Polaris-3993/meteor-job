...
Template.allCompanies.events({
    'click #select-all': function (event, tmpl) {
        ...
        var allChecked = $('tbody input[type="checkbox"]', '#companies-table').prop('checked', this.checked);
    ...
    'click #archive-companies': function (event, tmpl) {
        event.preventDefault();
        var table = $('#companies-table').DataTable();
        var checkedCompanies = tmpl.checkedCompanies.array();
        Session.set('companiesFormChanged',  false);
        Meteor.call('archiveCompanies', checkedCompanies, function (error, result) {
            if(!error){
                VZ.notify('Archived');
                tmpl.checkedCompanies.clear();
